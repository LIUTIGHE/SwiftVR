#!/usr/bin/env python3
"""Full-joint recovery training for the Stage-B1 Moderate TC Decoder v1.

The source checkpoint is an already trained compact ResizeConv decoder (recommended:
R4-Joint best).  It is widened in a mathematically function-preserving way to:

    condition channels      32 -> 128
    outer stage widths      (192,128,64,32) unchanged
    compact internal widths (80,48,24,16) -> (128,96,48,24)
    output head             nearest x2 + shared 3x3 RGB Conv unchanged

The target is ~66.548 GMAC/output-frame at 1920x1088.  The entire widened decoder
is jointly optimized with the same semantic LR groups and the same formal cached
z_SR + ReAE-teacher objective used by the matched R4-Joint experiment.  Epoch-0
validation is always written and should numerically reproduce the source checkpoint.
"""
...
from tools import train_tiny_decoder_matched_joint_recovery_ddp as matched
...
TRAINER_ID = "swiftvr_stage_b1_moderate_tc_decoder_recovery_ddp_v1"
TRAINABLE_SCOPE = "full_moderate_decoder_joint"
ESTIMATED_GMAC_PER_OUTPUT_FRAME_1920X1088 = 66.54799872


def build_parser() -> argparse.ArgumentParser:
    parser = formal.build_parser()
    parser.description = __doc__
    parser.add_argument("--resize-mode", default=MODERATE_RESIZE_MODE, choices=("nearest",))
    parser.add_argument("--condition-input-learning-rate", type=float, default=1e-5)
    parser.add_argument("--early-learning-rate", type=float, default=5e-6)
    parser.add_argument("--stage2-learning-rate", type=float, default=1e-5)
    parser.add_argument("--transition23-learning-rate", type=float, default=1e-5)
    parser.add_argument("--stage3-learning-rate", type=float, default=2e-5)
    ...
    value.update(
        {
            ...
            "blocks_per_stage": list(MODERATE_BLOCKS_PER_STAGE),
            "block_internal_channels": list(MODERATE_INTERNAL_CHANNELS),
            "resize_mode": args.resize_mode,
            "estimated_gmac_per_output_frame_1920x1088": ESTIMATED_GMAC_PER_OUTPUT_FRAME_1920X1088,
            "parameter_group_learning_rates": _learning_rates(args),
            "condition_dropout_probability": 0.0,
            "explicit_phase_loss": False,
        }
    )
    # The formal fingerprint contains a single learning_rate that is unused here;
    # remove it so resume is controlled only by the semantic group rates above.
    value.pop("learning_rate", None)
    return value


def _build_optimizer(
    groups: dict[str, list[torch.nn.Parameter]], args: argparse.Namespace
) -> torch.optim.AdamW:
    rates = _learning_rates(args)
    param_groups: list[dict[str, object]] = []
    for name, lr in rates.items():
        params = groups.get(name, [])
        if not params:
            raise RuntimeError(f"Optimizer group {name!r} is empty")
        bad = sorted({str(p.dtype) for p in params if p.dtype != torch.float32})
        if bad:
...
def _assert_target_topology(model: ModerateResizeConvTinyConditionalDecoder) -> None:
    if model.block_mode != "compact":
        raise ValueError(f"Moderate v1 requires compact blocks, got {model.block_mode!r}")
    if int(model.condition_channels) != MODERATE_CONDITION_CHANNELS:
        raise ValueError(
            f"Expected condition_channels={MODERATE_CONDITION_CHANNELS}, got {model.condition_channels}"
        )
    if tuple(model.channels) != MODERATE_CHANNELS:
        raise ValueError(f"Expected channels={MODERATE_CHANNELS}, got {model.channels}")
    if tuple(model.blocks_per_stage) != MODERATE_BLOCKS_PER_STAGE:
        raise ValueError(
            f"Expected blocks_per_stage={MODERATE_BLOCKS_PER_STAGE}, got {model.blocks_per_stage}"
        )
    if tuple(model.block_internal_channels) != MODERATE_INTERNAL_CHANNELS:
        raise ValueError(
            f"Expected internal widths={MODERATE_INTERNAL_CHANNELS}, got {model.block_internal_channels}"
        )
    if model.resize_mode != MODERATE_RESIZE_MODE:
        raise ValueError(f"Expected resize_mode={MODERATE_RESIZE_MODE!r}, got {model.resize_mode!r}")
...
def _load_initial_model(
    ...
    *,
    ...
    model, report = ModerateResizeConvTinyConditionalDecoder.from_resizeconv_pretrained(
        init_decoder,
        device=device,
        dtype=dtype,
    )
    _assert_target_topology(model)
    return model, report
...
def _load_resume_model(
    ...
    *,
    ...
    model = ModerateResizeConvTinyConditionalDecoder.from_pretrained(
...
def main() -> int:
    ...
    try:
        ...
        trainable_report, groups = matched._set_joint_trainable(tiny, variant="r4")
        trainable_report = {**trainable_report, "scope": TRAINABLE_SCOPE, "variant": "moderate_v1"}
        ...
        if args.resume is None:
            if rank == 0:
                ...
                print(
                    json.dumps(
                        {"phase": "initial_moderate_validation", **initial_val},
                        sort_keys=True,
                    ),
                    flush=True,
                )
