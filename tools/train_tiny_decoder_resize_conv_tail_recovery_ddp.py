#!/usr/bin/env python3
"""Tail recovery for the Stage-B1 resize-conv Tiny Decoder.

This trainer is intentionally isolated from the canonical/head-only Tiny Decoder
trainers.  It starts from an already-trained ResizeConvTinyConditionalDecoder
checkpoint (recommended: ResizeConv epoch 4) and optimizes only the late spatial
reconstruction tail:

    decoder stage2 blocks @ 96x96
    transition stage2 -> stage3 (nearest x2 + TGrow + Conv)
    decoder stage3 blocks @ 192x192
    resize-conv RGB output head @ 384x384

Condition projection, input mapping, decoder stage0/stage1, and the stage1->stage2
transition remain frozen.  The experiment therefore tests whether the strong
output-space p8 artifact can be reduced by adapting the decoder region whose
current-resolution p2 feature bias maps to approximately period 8 after the two
remaining x2 spatial expansions.

Frozen cached z_SR latents, frozen ReAE teacher, datasets, validation metrics, and
the dual GT/teacher MSE+LPIPS objective are inherited from the formal Stage-B1
trainer.  No explicit phase loss is introduced in this first tail-recovery test.
"""
...
TRAINER_ID = "swiftvr_stage_b1_tiny_decoder_resize_conv_tail_recovery_ddp_v1"
TRAINABLE_SCOPE = "decoder_stage2_transition23_stage3_output_head"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-5,
        help="LR for decoder stage2 compact blocks.",
    )
    parser.add_argument(
        "--transition-learning-rate",
        type=float,
        default=1e-5,
        help="LR for trainable tensors in the stage2->stage3 transition.",
    )
    parser.add_argument(
        "--stage3-learning-rate",
        type=float,
        default=3e-5,
        help="LR for decoder stage3 compact blocks.",
    )
    parser.add_argument(
        "--head-learning-rate",
        type=float,
        default=1e-4,
        help="LR for the resize-conv RGB head.",
    )
    return parser
...
def _decoder_layout(model: ResizeConvTinyConditionalDecoder) -> dict[str, tuple[int, ...] | int]:
    """Return semantic decoder spans for the fixed four-stage topology."""
    blocks = tuple(int(value) for value in model.blocks_per_stage)
    if len(blocks) != 4:
        raise ValueError(f"Expected four decoder stages, got {blocks}")

    # decoder = Clamp, input Conv, ReLU, then
    # [stage0, trans01, stage1, trans12, stage2, trans23, stage3, output ReLU].
    index = 3
    stage0 = tuple(range(index, index + blocks[0]))
    index += blocks[0]
    transition01 = tuple(range(index, index + 3))
    index += 3
    stage1 = tuple(range(index, index + blocks[1]))
    index += blocks[1]
    transition12 = tuple(range(index, index + 3))
    index += 3
    stage2 = tuple(range(index, index + blocks[2]))
    index += blocks[2]
    transition23 = tuple(range(index, index + 3))
    index += 3
    stage3 = tuple(range(index, index + blocks[3]))
    index += blocks[3]
    output_relu = index
    index += 1

    if index != len(model.decoder):
        raise RuntimeError(
            "Unexpected ResizeConv Tiny decoder topology: "
            f"semantic layout consumes {index} layers but decoder has {len(model.decoder)}"
        )
    return {
        "stage0": stage0,
        "transition01": transition01,
        "stage1": stage1,
        "transition12": transition12,
        "stage2": stage2,
...
def _set_tail_trainable(
    ...
    for group_name, layout_name in (
        ...
        for index in indices:
            for parameter in model.decoder[index].parameters(recurse=True):
                parameter.requires_grad_(True)
                groups[group_name].append(parameter)

    for parameter in model.output_head.parameters():
        parameter.requires_grad_(True)
        groups["output_head"].append(parameter)

    # Ensure optimizer groups are disjoint and exactly cover requires_grad=True.
    ...
    grouped_ids = set().union(*(ids_by_group.values()))
    if trainable_ids != grouped_ids:
        raise RuntimeError(
            "Tail-recovery trainable/grouped parameter mismatch: "
            f"trainable={len(trainable_ids)} grouped={len(grouped_ids)}"
        )
    ...
    for group_name, parameters in groups.items():
        group_report[group_name] = {
            ...
            "parameter_names": [name_by_id[id(parameter)] for parameter in parameters],
        }
...
def _build_grouped_adamw(
    ...
    lrs = {
        ...
        "transition23": float(args.transition_learning_rate),
        "stage3": float(args.stage3_learning_rate),
        "output_head": float(args.head_learning_rate),
    }
    param_groups: list[dict[str, object]] = []
    for name in ("stage2", "transition23", "stage3", "output_head"):
        parameters = groups[name]
        if not parameters:
            # nearest upsampling has no parameters, but each semantic group as a
            # whole should still contain trainable tensors in the current model.
        ...
        if bad:
            raise RuntimeError(
                f"Optimizer group {name} must contain FP32 parameters, found {bad}"
            )
        param_groups.append({"params": parameters, "lr": lrs[name], "group_name": name})
...
def main() -> int:
    ...
    try:
        ...
        if args.resume is not None:
            ...
            # Unlike head-only surgery training, this run starts from an already
            # trained ResizeConv checkpoint (R4 recommended), not a PixelShuffle
            # checkpoint and not a new phase-averaging surgery.
            tiny = ResizeConvTinyConditionalDecoder.from_pretrained(
                init_decoder,
                device=device,
                dtype=dtype,
        ...
        trainable_report, groups = _set_tail_trainable(tiny)
        cast_report = cast_trainable_parameters(tiny, dtype=torch.float32)
        tiny.train()
        optimizer = _build_grouped_adamw(groups, args)
        ...
        # Epoch-0 validation is the exact supplied ResizeConv initialization.
        if args.resume is None:
            if rank == 0:
                ...
                print(
                    json.dumps(
                        {
                            "phase": "initial_tail_recovery_validation",
                            "trainable_scope": TRAINABLE_SCOPE,
                            **initial_val,
                        },
                        sort_keys=True,
