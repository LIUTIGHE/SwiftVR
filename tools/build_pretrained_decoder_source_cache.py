#!/usr/bin/env python3
"""Cache restored latents from a full pretrained prompt-free SwiftVR checkpoint.

This is the full-checkpoint counterpart of ``build_tiny_decoder_source_cache.py``.
The legacy builder targets Stage-A delta checkpoints; this tool targets reloadable
transformer checkpoints (dense or sparse-MoE) that contain ``transformer/``.

The primary M8-B use case is:

    M8-A D1024/L20 checkpoint -> z_M8 = z_LQ - v_M8 -> immutable decoder cache

Both the frozen original ReAE teacher decoder and Decoder76 then consume exactly
this same cached z_M8 during teacher-only warm-up.  The cache intentionally keeps
the established ``swiftvr_stage_b1_sr_latent`` format so the validated structured
ReAE trainer can be reused without special-case data loading.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--selection-mode", choices=tuple(sorted(SELECTION_MODES)), default="all")
    ...
    p.add_argument("--transformer-type", choices=("auto", "dense", "moe"), default="auto")
...
def _detect_transformer_type(root: Path, subfolder: str) -> str:
    ...
    return "moe" if "MoE" in class_name or moe_keys.issubset(config) else "dense"
...
def _load_transformer(
    ...
    *,
    ...
    kind = _detect_transformer_type(root, subfolder) if transformer_type == "auto" else transformer_type
    cls = WanTransformer3DModelPromptFreeNoTimeMoE if kind == "moe" else WanTransformer3DModelPromptFreeNoTime
...
def main() -> int:
    ...
    for name in ("views_per_record", "batch_size", "crop_size", "scale", "clip_length"):
    ...
    if output.exists() and any(output.iterdir()):
        if not args.overwrite:
            raise FileExistsError(f"Decoder latent cache is not empty: {output}")
    ...
    selected_record_indices = {int(index) // int(args.views_per_record) for index in selected_indices}
    ...
    with torch.inference_mode():
        for batch_cpu in loader:
            ...
            with torch.autocast(
                ...
                velocity = extract_transformer_sample(transformer(z_lq, return_dict=True))
            ...
            for local_index in range(int(z_sr.shape[0])):
                ...
                latent = z_sr[local_index].detach().to(device="cpu", dtype=torch.float16).contiguous()
                relative_file = f"samples/{distillation_index:08d}_{identity['key']}.safetensors"
                ...
                if processed == 1 or processed % 32 == 0 or processed == len(selected_indices):
                    print(
                        ...
                        f"dataset_index={distillation_index} {identity['record_uid']} "
                        f"view={identity['view_index']}",
                        flush=True,
                    )
    ...
    metadata_sha = sha256_file(source_metadata) if source_metadata.is_file() else sha256_file(source_config)
    metadata: dict[str, object] = {
        "format_version": TINY_DECODER_CACHE_FORMAT_VERSION,
        "kind": "swiftvr_stage_b1_sr_latent",
        "latent_source_kind": "full_pretrained_transformer",
        "latent_semantics": "z_lq_minus_source_velocity",
        ...
        "transformer_config_sha256": sha256_file(source_config),
